...
class TwoFactorAuthenticationApiController extends AbstractController
{
    ...
    #[Route(path: '/api/rl-2fa/generate-secret', name: 'api.action.rl-2fa.generate-secret', methods: ['GET'])]
    public function generateSecret(Request $request, Context $context): JsonResponse
    {
        ...
        $qrUrl = $this->totpService->getQrCodeUrl($company, $request->get('holder', ''), $secret);
    ...
    #[Route(path: '/api/rl-2fa/validate-secret', name: 'api.action.rl-2fa.validate-secret', methods: ['POST'])]
    public function validateSecret(Request $request, Context $context): JsonResponse
    {
        if (empty($request->get('secret')) || empty($request->get('code'))) {
            return new JsonResponse(['status' => 'error', 'error' => 'Secret or code empty'], 400);
        }
        ...
        return new JsonResponse(['status' => 'error', 'error' => 'Secret and code not correct'], 400);
    }
}
